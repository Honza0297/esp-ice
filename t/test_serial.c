 ...
 *
 * We don't want a real ESP to run tests, so we lean on POSIX pseudo-
 * terminals: @c posix_openpt creates a master fd; ptsname() gives us
 * the slave-side device path which @ref serial_open can treat as a
 * regular tty.  Writing to the master is observed by serial_read on
 * the slave, and vice versa -- enough to exercise the data path,
 * timeouts, and the modem-control ioctls (which succeed on Linux
 * ptys even though they don't toggle a physical line).
 */

/* grantpt / unlockpt / ptsname need XSI extensions beyond POSIX.1-2001. */
...
/*
 * Open a master pty and return its fd via @p master_fd; fill @p slave
 * (sized @p slave_sz) with the slave-side path that @ref serial_open
 * can use.  Dies on failure so the test aborts with a clear error.
 */
...
int main(void)
{
	...
	tap_check(s.fd >= 0);
	tap_check(strcmp(s.path, slave_path) == 0);
	tap_done("serial_open on a pty slave succeeds");

	tap_check(serial_set_baud(&s, 115200) == 0);
	tap_check(s.baud == 115200);
	tap_done("serial_set_baud accepts 115200 and caches it");
	...
	/* Modem control: on a pty the ioctl often returns -ENOTTY because
	 * there is no physical line to toggle.  Real USB-serial adapters
	 * accept it; for this test we only verify the API is callable and
	...
	/* Flush: queue bytes on master, then flush — next read should
	 * time out instead of returning the stale data. */
	...
	serial_close(&s);
	tap_check(s.fd == -1);
	tap_done("serial_close resets the fd to -1");

	/* Closing an already-closed port is safe. */
	serial_close(&s);
	tap_done("serial_close is idempotent");
